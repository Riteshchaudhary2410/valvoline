...
import { RECENT_ORDERS, getRecommendedByOrderHistory, getProductBySlug } from '@/lib/catalog';
...
export default function DashboardPage() {
  ...
  const totalValue = RECENT_ORDERS.reduce((sum, order) => sum + order.total, 0);
  const roleLabel = user?.role === 'GARAGE' || user?.role === 'MECHANIC' ? 'B2B buyer' : 'Retail customer';

  const handleOrderAgain = (orderId: string) => {
    const order = RECENT_ORDERS.find((item) => item.id === orderId);
    if (!order) return;

    getRecommendedByOrderHistory(order).forEach(({ product, quantity }) => {
                  ...
                  {RECENT_ORDERS.map((order) => (
                    <div key={order.id} className="rounded-3xl border border-white/10 bg-black/20 p-5">
                      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div>
                          <p className="text-sm uppercase tracking-[0.24em] text-gray-500">{order.orderNumber}</p>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {order.items.map((item) => {
                          ...
                          return (
                            <span key={`${order.id}-${item.productSlug}`} className="badge badge-secondary">
                              {product?.name || item.productSlug} x {item.quantity}
                            </span>
                          );
                        })}
                      </div>
